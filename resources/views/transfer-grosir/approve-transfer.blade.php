    ...
    <table class="table tabelan table-bordered table-hover dt-responsive" id="detail-req" >
        ...
        <tbody>
            ...
            <tr>
                ...
                {{$data->tidt_qty}}</td>
                <td class="text-right">
                    <input onkeyup="aktifSend('{{$data->i_id}}');HitungQtyApprove('{{$data->i_id}}');" id="qtyAppr-{{$data->i_id}}" type="text" name="qtyAppr[]"  class="form-control text-right currency-x" value="{{$data->tidt_qty_appr}}">
                </td>
                <td class="text-right">
                    <input onkeyup="HitungQtySend('{{$data->i_id}}');" id="qtySend-{{$data->i_id}}"
                    @if($data->tidt_qty_appr=='' && $data->tidt_qty_send=='') readonly="" @endif
                    type="text" name="qtySend[]"  class="form-control text-right currency-x" value="{{$data->tidt_qty_send}}" >
                </td>
            </tr>
            @endforeach
        </tbody>
